...
function QuotationProduct({
  ...
  return (
    <section className={`w-[20.5rem] h-[9.25rem] lg:w-[21.75rem] flex flex-row border-2 border-beige ${className}`} >
      ...
      <section className='w-3/5 h-full flex flex-col gap-2 lg:gap-4 border px-3 py-2'>
        ...
        <section className='flex flex-col '>
          <span className='text-title my-auto'>DESCRIPCIÓN:</span>
          <p className='text-input text-left'>{productDescription}</p>
        </section>

        <section className='flex flex-row justify-between'>
          <span
          className='text-title underline underline-offset-1 hover:cursor-pointer my-auto'
          onClick={() => { }}
          >
            Ver Cotización
          </span>
